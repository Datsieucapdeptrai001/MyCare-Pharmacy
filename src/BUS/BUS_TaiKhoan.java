 ...
 * Tầng BUS nằm giữa giao diện (GUI) và tầng truy xuất DB (DAO).
 * Mọi thao tác với tài khoản đều đi qua đây trước khi xuống DAO.
 */
public class BUS_TaiKhoan {
     ...
     * Đặc biệt: Nếu DB đang lưu mật khẩu thô (chưa băm từ hệ thống cũ),
     * hàm sẽ tự động băm và cập nhật lại DB ngay lần đầu đăng nhập đúng.
     *
     ...
     * Tự động băm mật khẩu thô trước khi lưu xuống DB.
     *
     * @param tk Đối tượng TaiKhoan cần thêm (mật khẩu vẫn còn dạng thô)
     * @return true nếu thêm thành công, false nếu trùng tên hoặc mật khẩu không hợp lệ
     */
    ...
    // =========================================================
    /**
     * Cập nhật thông tin tài khoản (tên đăng nhập, mật khẩu) theo mã nhân viên.
     * Hỗ trợ 3 tình huống:
     *   - Quản lý để trống ô mật khẩu: giữ nguyên mật khẩu cũ
     *   - Quản lý nhập mật khẩu mới thô: băm rồi lưu
     *   - Mật khẩu đã là bcrypt (bắt đầu bằng $2, dài ≥ 60 ký tự): lưu thẳng
     *
    ...
    public boolean capNhatTaiKhoanTheoMaNV(String maNV, String tenDangNhap, String matKhauNhapVao) {
        ...
        // Kiểm tra mật khẩu có phải bcrypt chưa (bcrypt bắt đầu bằng "$2" và dài ≥ 60 ký tự)
    ...
    // =========================================================
    /**
     * Cho phép người dùng đổi mật khẩu của chính mình.
     * Phải xác thực đúng mật khẩu hiện tại trước, sau đó mới cập nhật mật khẩu mới.
     *
    ...
    // =========================================================
    /**
     * Cập nhật mật khẩu mới thông qua email (chức năng quên mật khẩu).
     * Không cần xác thực mật khẩu cũ, chỉ cần email hợp lệ.
     *
     ...
     * Điều kiện: tối thiểu 8 ký tự, phải có ít nhất 1 chữ cái và 1 chữ số.
     *
     * @param matKhauMoi Chuỗi mật khẩu cần kiểm tra
     * @return true nếu hợp lệ
     */
    public boolean validateMatKhauMoi(String matKhauMoi) {
        if (matKhauMoi == null || matKhauMoi.length() < 8) return false; // Quá ngắn
        ...
        for (char c : matKhauMoi.toCharArray()) {
            if (Character.isLetter(c)) hasLetter = true; // Tìm thấy chữ cái
            if (Character.isDigit(c)) hasDigit = true;   // Tìm thấy chữ số
        }
     ...
     * @return TaiKhoan với NhanVienId đã được nạp đầy đủ dữ liệu, hoặc null nếu không tìm thấy
     */
