...
public class BUS_SanPham {
    ...
    public List<SanPham> traCuuSanPham(String tuKhoa) {
        ...
        for (SanPham sp : ds) {
            ...
            String id = safe(sp.getId()).toLowerCase();
            String ten = safe(sp.getTen()).toLowerCase();
            String vietTat = safe(sp.getTenVietTat()).toLowerCase();
            String hoatChat = safe(sp.getHoatChat()).toLowerCase();

            if (id.contains(kw)
                    || ten.contains(kw)
                    || vietTat.contains(kw)
                    || hoatChat.contains(kw)) {
                ketQua.add(sp);
            }
        }
