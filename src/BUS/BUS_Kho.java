...
public class BUS_Kho {
    ...
    public List<LoHang> canhBaoHangSapHetHan(int soNgayCanhBao) {
        ...
        for (LoHang lh : dsToanBoLo) {
            ...
            if (lh.getNgayHetHan().isAfter(hienTai) && lh.getNgayHetHan().isBefore(mocCanhBao)
                    && lh.getSoLuongLoHang() > 0 && lh.getTrangThai() != TrangThaiLoHang.HET_HAN) {
                dsCanhBao.add(lh);
            }
        }
        return dsCanhBao;
    ...
    public boolean kiemKeKho() {
        try { return daoLoHang.dongBoTrangThaiLoHang(); } 
        catch (Exception e) { e.printStackTrace(); return false; }
    }

    public List<LoHang> layDSLoHang() { return daoLoHang.layDSLoHang(false); }
    public List<LoHang> layDSLoHang(boolean hienLoAn) { return daoLoHang.layDSLoHang(hienLoAn); }
    public List<LoHang> layDSLoHangDaAn() { return daoLoHang.layDSLoHangDaAn(); }
    ...
    public boolean themLoHang(LoHang loHang) {
        ...
        if (daoSanPham.laSanPhamDaAn(maSP)) return false;
        if (loHang.getNgayNhap().isAfter(loHang.getNgayHetHan())) return false;
        ...
        if (loCu.getTrangThai() != TrangThaiLoHang.AN) return false;
    ...
    public boolean khoiPhucLoHang(String maLoHang) {
        ...
        if (lo.getSanPhamId() != null && !isBlank(lo.getSanPhamId().getId())) {
            if (daoSanPham.laSanPhamDaAn(lo.getSanPhamId().getId().trim())) return false;
        }
        return daoLoHang.khoiPhucLoHang(maLoHang);
    }

    public boolean capNhatSoLuongTon(String maLoHang, int soLuongMoi) {
        if (isBlank(maLoHang) || soLuongMoi < 0) return false;
        LoHang lo = daoLoHang.getLoHangTheoId(maLoHang);
        if (lo == null) return false;
    ...
    private TrangThaiLoHang suyRaTrangThai(int soLuong, LocalDateTime ngayHetHan) {
        ...
        if (ngayHetHan != null && ngayHetHan.isBefore(now)) return TrangThaiLoHang.HET_HAN;
        if (soLuong <= 0) return TrangThaiLoHang.HET_HANG;
    ...
    public boolean xuatHuyKho(List<Object[]> danhSachXuat, String nguoiThucHien) {
        if (danhSachXuat == null || danhSachXuat.isEmpty() || nguoiThucHien == null || nguoiThucHien.isEmpty()) return false;
