...
public class ManHinhXuatKho extends JPanel {
    ...
    private JPanel createInputForm() {
        ...
        pnlWrapper.setPreferredSize(new Dimension(360, 0));
        ...
        pnlMaLo.setMaximumSize(new Dimension(340, 38));
        pnlMaLo.setPreferredSize(new Dimension(340, 38));
        ...
        pnlMaLo.add(btnCheck, BorderLayout.EAST);
        ...
        cbLyDo.setMaximumSize(new Dimension(340, 38));
        cbLyDo.setPreferredSize(new Dimension(340, 38));
        ...
        btnAdd.setPreferredSize(new Dimension(340, 45)); 
    ...
    private JPanel createCartTable() {
        ...
        JLabel lblTitle = new JLabel("Danh sách Lô hàng chờ xuất");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 18));
        ...
        JButton btnXacNhan = createHoverButton("XÁC NHẬN XUẤT KHO", DANGER, DANGER_HOVER, Color.WHITE);
    ...
    private void setupAutoSuggest() {
        ...
        suggestList.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { if (e.getClickCount() == 1) chonGoiY(); }
        });
    }
    ...
    private void kiemTraMaLo() {
        ...
        if (ma.isEmpty()) {
            showCustomNotification("Thiếu thông tin", "Vui lòng nhập hoặc chọn Mã lô hàng cần xuất.", "WARNING");
            return;
        }

        LoHang loTimThay = null;
        for (LoHang lh : cacheDanhSachLo) {
            if (lh.getSoLoHang().equalsIgnoreCase(ma)) { loTimThay = lh; break; }
        }

        if (loTimThay == null) {
            showCustomNotification("Không tìm thấy lô hàng", "Mã lô '" + ma + "' không tồn tại trong hệ thống.\nVui lòng kiểm tra lại.", "ERROR");
            resetThongTinSP();
            return;
        }
        ...
        if (ds != null && !ds.isEmpty()) {
            ...
            for (DonViDoLuong dv : ds) if (dv.getChuyenDoiSangDonViCoBan() == 1.0) { dvStr = " " + dv.getTen(); break; }
        }
    ...
    private void themVaoDanhSach() {
        if (lblTenSP.getText().equals("-- Chưa chọn --")) {
            showCustomNotification("Chưa chọn sản phẩm", "Vui lòng nhập và kiểm tra Mã lô hợp lệ trước khi thêm.", "WARNING");
            return;
        }
        ...
        try {
            ...
            if (slXuat <= 0) {
                showCustomNotification("Số lượng không hợp lệ", "Số lượng xuất kho phải lớn hơn 0.", "ERROR");
                return;
            }
            if (slXuat > currentTonKho) {
                showCustomNotification("Vượt quá tồn kho", "Số lượng yêu cầu vượt quá số tồn khả dụng (" + currentTonKho + ").", "ERROR");
                return;
            }

            tableModel.addRow(new Object[]{txtMaLo.getText().trim(), lblTenSP.getText(), slXuat, lblDonVi.getText().trim(), cbLyDo.getSelectedItem().toString(), ""});
            resetFormNhap();
        } catch (NumberFormatException e) {
            showCustomNotification("Sai định dạng", "Số lượng xuất không hợp lệ. Vui lòng chỉ nhập số nguyên.", "ERROR");
        }
    }

    private void hoanTatXuatKho() {
        if (tableModel.getRowCount() == 0) {
            showCustomNotification("Danh sách trống", "Chưa có lô hàng nào trong danh sách chờ xuất.", "WARNING");
            return;
        }
    ...
    private void resetFormNhap() {
        txtMaLo.setText(""); txtSoLuong.setText(""); txtGhiChu.setText("");
        cbLyDo.setSelectedIndex(0); resetThongTinSP();
    }
    
    private void resetThongTinSP() {
        lblTenSP.setText("-- Chưa chọn --"); lblTonKhoHienTai.setText("0");
        lblDonVi.setText(" Đơn vị"); currentTonKho = 0;
    }
    ...
    private JTextField createTextField() {
        ...
        txt.setBorder(BorderFactory.createCompoundBorder(new RoundedLineBorder(BORDER_COLOR, 1, 8), new EmptyBorder(5, 12, 5, 12)));
    ...
    private void showCustomNotification(String titleText, String message, String type) {
        ...
        scroll.setBounds(95, 20, 305, 80); scroll.setBorder(null);
    ...
    private static class SmoothShadowBorder extends AbstractBorder {
        private final Color shadow; private final int radius;
    ...
    private static class CompoundRoundBorder extends AbstractBorder {
        private final AbstractBorder outer; private final Insets inner;
        public CompoundRoundBorder(AbstractBorder o, Insets i) { outer = o; inner = i; }
        @Override public Insets getBorderInsets(Component c) {
            Insets o = outer.getBorderInsets(c); return new Insets(o.top + inner.top, o.left + inner.left, o.bottom + inner.bottom, o.right + inner.right);
        }
