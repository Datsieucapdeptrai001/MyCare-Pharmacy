...
public class ManHinhChinh extends JPanel {
    ...
    // Biến lưu trữ số lượng tờ tiền cho 9 mệnh giá
    ...
    private JPanel createTopHeader() {
        ...
        btnTongQuan = createTabButton("📊 Tổng quan", true);
        btnDoiChieu = createTabButton("💲 Đối chiếu doanh thu", false);
    ...
    private void openKetCaDialog() {
        ...
        // Nhãn hiển thị Tổng tiền ở dưới cùng
        ...
        for (int i = 0; i < menhGiaStr.length; i++) {
            final int index = i; // Biến final để dùng trong MouseListener
            
            JPanel box = new JPanel(new BorderLayout());
            box.setBorder(BorderFactory.createLineBorder(boxColors[i], 1));
            box.setBackground(Color.WHITE);
            box.setCursor(new Cursor(Cursor.HAND_CURSOR)); // Đổi chuột thành hình bàn tay
            ...
            // Xử lý sự kiện click đếm tiền
            box.addMouseListener(new MouseAdapter() {
                @Override
                public void mouseClicked(MouseEvent e) {
                    if (SwingUtilities.isLeftMouseButton(e)) {
                        soLuongTien[index]++; // Chuột trái -> Cộng 1
                    } else if (SwingUtilities.isRightMouseButton(e)) {
                        if (soLuongTien[index] > 0) soLuongTien[index]--; // Chuột phải -> Trừ 1
                    }
    ...
    // Hàm phụ trợ tạo Panel Tổng kết doanh thu (Y hệt ảnh thiết kế)
    ...
    private JPanel createStatBox(String title, String value, String bgColor, String fgColor) {
        ...
        box.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(Color.decode(fgColor), 1), // Viền đồng màu chữ
