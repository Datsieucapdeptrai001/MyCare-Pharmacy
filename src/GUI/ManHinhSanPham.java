...
public class ManHinhSanPham extends JPanel {
    ...
    private JButton btnNhapTop;
    private JButton btnXuatTop;
    ...
    public void setReadOnly(boolean isReadOnly) {
        ...
        if (btnCapNhatBottom != null) btnCapNhatBottom.setEnabled(canEdit);
        if (btnXoaBottom     != null) btnXoaBottom.setEnabled(canEdit);
        if (btnLuuBottom     != null) btnLuuBottom.setEnabled(canEdit);
        if (btnXacNhanThem   != null) btnXacNhanThem.setEnabled(canEdit);

        // Khóa các nút TopBar — nhân viên không được Thêm / Nhập / Xuất Excel
        if (btnNhapTop != null) btnNhapTop.setEnabled(canEdit);
        if (btnXuatTop != null) btnXuatTop.setEnabled(canEdit);
        if (btnThem    != null) ((JButton) btnThem).setEnabled(canEdit);
    }
}