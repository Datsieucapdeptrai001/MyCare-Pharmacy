...
public class ManHinhSanPham extends JPanel {
	...
	public static class UnitOfMeasure {
		...
		public UnitOfMeasure(MeasurementName measurement, BigDecimal rate) {
			...
			this.baseUnitConversionRate = rate;
		}
	...
	public static class Product {
		...
		public Product(String id, String bc, ProductCategory cat, DosageForm frm, String nm, String sn, String mfg,
				String act, double v, String str, String desc, String baseUom) {
			this.id = id;
			this.category = cat;
			this.form = frm;
			this.name = nm;
			...
			this.activeIngredient = act;
			this.vat = v;
			this.strength = str;
			this.description = desc;
			this.baseUnitOfMeasure = baseUom;
		}
		...
		public Set<UnitOfMeasure> getUnitOfMeasureSet() {
			return unitOfMeasureSet;
		}

		public void setUnitOfMeasureSet(Set<UnitOfMeasure> uoms) {
			this.unitOfMeasureSet = uoms;
		}
		...
		public void setLotSet(Set<Lot> lots) {
			this.lotSet = lots;
		}
	}
	...
	private static final String[] COLS_COLLAPSED = { "Mã", "Mã vạch", "Nhóm bệnh lý", "Tên", "Loại", "Hoạt chất",
			"Giá bán" };
	private static final String[] COLS_EXPANDED = { "Mã", "Mã vạch", "Nhóm bệnh lý", "Tên", "Loại", "Hoạt chất",
			"Dạng bào chế", "NSX", "VAT", "Giá bán" };
	private static final String[] DANH_SACH_NHOM_BENH_LY = { "Ho – Đờm – Viêm họng", "Sốt – Cảm cúm – Sổ mũi",
			"Đau đầu – Giảm đau – Hạ sốt", "Dạ dày – Tiêu hóa – Đại tràng", "Dị ứng – Mẩn ngứa – Mề đay",
			"Mắt – Tai – Mũi", "Thuốc bổ – Vitamin – Khoáng chất", "Dược mỹ phẩm – Da liễu", "Xương khớp – Gút",
			"Tim mạch – Huyết áp", "Đái tháo đường", };
	...
	private JPanel createTabQuanLyLieu() {
		...
		tblDanhSachLieuNangCao.getSelectionModel().addListSelectionListener(e -> {
			if (!e.getValueIsAdjusting())
				onChonMauLieuTuDanhSach();
		});
		...
		for (String nb : DANH_SACH_NHOM_BENH_LY)
			cbNhomBenhLieuNangCao.addItem(nb);
			...
			if ("+ Thêm nhóm mới...".equals(cbNhomBenhLieuNangCao.getSelectedItem())) {
				...
				if (tenMoi != null && !tenMoi.isEmpty()) {
					...
					for (int i = 0; i < cbNhomBenhLy.getItemCount(); i++) {
						if (cbNhomBenhLy.getItemAt(i).equalsIgnoreCase(tenMoi)) {
							existsSP = true;
							break;
						}
					}
		...
		pnlDoses.add(new JLabel("Sáng:"));
		pnlDoses.add(txtSangLieu = new JTextField("1", 3));
		pnlDoses.add(new JLabel("Trưa:"));
		pnlDoses.add(txtTruaLieu = new JTextField("0", 3));
		pnlDoses.add(new JLabel("Chiều:"));
		pnlDoses.add(txtChieuLieu = new JTextField("0", 3));
		pnlDoses.add(new JLabel("Tối:"));
		pnlDoses.add(txtToiLieu = new JTextField("1", 3));
		pnlDoses.add(new JLabel("Số ngày:"));
		pnlDoses.add(txtSoNgayLieu = new JTextField("5", 3));
		...
		cbCachDungLieuNangCao = new JComboBox<>(
				new String[] { "Sau ăn 30 phút", "Trước ăn 30 phút", "Trong bữa ăn", "Lúc đói" });
		...
		String[] colsChiTiet = { "Mã SP", "Tên Thuốc", "ĐVT", "Sáng", "Trưa", "Chiều", "Tối", "Cách dùng", "Ngày",
				"Tổng SL", "Thành tiền", "Xóa" };
		...
		tblChiTietLieu.getColumnModel().getColumn(COL_L_XOA).setCellRenderer(new DefaultTableCellRenderer() {
			@Override
		...
		tblChiTietLieu.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (viewRow >= 0 && col == COL_L_XOA) {
					// [FIX 4] Chuyển index giao diện → index model để tránh IndexOutOfBounds khi
					// sort
					int row = tblChiTietLieu.convertRowIndexToModel(viewRow);
					if (row < 0 || row >= dsChiTietLieuLuuTru.size())
						return;
		...
		btnDeXuat.setToolTipText(
				"<html><b>Lấy giá đề xuất</b><br>" + "Tính tổng thành tiền của tất cả thuốc trong toa,<br>"
	...
	private void khoiTaoPopupTimThuoc() {
		...
		listPopupThuoc.setCellRenderer(new javax.swing.ListCellRenderer<String>() {
			@Override
			public Component getListCellRendererComponent(javax.swing.JList<? extends String> list, String value,
		...
		JScrollPane spPopup = new JScrollPane(listPopupThuoc, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	private JPanel createTopbar() {
		...
		pnl.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, COLOR_BORDER),
		...
		JButton btnTim = createBtnWithIcon("Tìm", "#1D68B2", new MenuIcon("SEARCH"));
		btnTim.setPreferredSize(new Dimension(95, 36));
		JButton btnLamMoi = createBtnWithIcon("Làm mới", "#64748B", new MenuIcon("REFRESH"));
		btnLamMoi.setPreferredSize(new Dimension(115, 36));
		JButton btnXuatTop = createBtnWithIcon("Xuất Excel", "#22C55E", new MenuIcon("EXPORT"));
		btnXuatTop.setPreferredSize(new Dimension(130, 36));
		JButton btnNhapTop = createBtnWithIcon("Nhập Excel", "#0EA5E9", new MenuIcon("IMPORT"));
		btnNhapTop.setPreferredSize(new Dimension(130, 36));
		JButton btnThem = createBtnWithIcon("Thêm mới", "#E11D48", new MenuIcon("ADD"));
		btnThem.setPreferredSize(new Dimension(125, 36));
		...
		pnlRight.add(btnXuatTop);
		pnlRight.add(btnNhapTop);
	...
	private void doSearch() {
		...
		for (Object[] row : allDataMock) {
			// row: [0]=mã, [1]=tên, [2]=loại, [3]=hoạtChất, [4]=dạng, [5]=nsx, [6]=vat,
			// [7]=giaBan, [8]=maVach, [9]=nhomBenhLy, [10]=tenVietTat
			boolean matchKey = keyword.isEmpty() || row[0].toString().toLowerCase().contains(keyword)
			...
			boolean matchDang = dangSelect.equals("Tất cả")
					|| (row.length > 4 && row[4].toString().equalsIgnoreCase(dangSelect));
			boolean matchNhom = nhomSelect.equals("Tất cả")
					|| (row.length > 9 && row[9].toString().equalsIgnoreCase(nhomSelect));
			if (matchKey && matchLoai && matchDang && matchNhom)
				currentFilteredData.add(row);
		}
		currentPageMock = 1;
		updatePagination();
	}
	...
	private void doSearchAndAutoOpen() {
		...
		if (kw.length() >= 6 && currentFilteredData.size() == 1) {
			...
			for (String mv : maVachSP.split(",")) {
				if (mv.trim().equalsIgnoreCase(kw)) {
					isExactMatch = true;
					break;
				}
			}
	...
	private JPanel createSidebarFilter() {
		...
		pnlLoaiBody = createRadioGroupPanel(
				new String[] { "Tất cả", "Thuốc kê đơn", "Thuốc không kê đơn", "Sản phẩm chức năng", "Mỹ phẩm" },
				bgLoai);
		...
		pnlDangBody = createRadioGroupPanel(new String[] { "Tất cả", "Viên nén", "Viên nang", "Viên sủi", "Thuốc bột",
				"Kẹo ngậm", "Dung dịch", "Hỗn dịch", "Thuốc nhỏ giọt", "Súc miệng" }, bgDang);
		...
		JScrollPane sp = new JScrollPane(scrollContent, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		sp.setBorder(null);
	...
	private JPanel createAccordionHeader(String title, boolean expanded, Runnable onToggle) {
		...
		if ("Loại".equals(title))
			btnToggleLoai = makeProxyButton(lblArrow);
		else if ("Dạng bào chế".equals(title))
			btnToggleDang = makeProxyButton(lblArrow);
		else if ("Nhóm bệnh lý".equals(title))
			btnToggleNhom = makeProxyButton(lblArrow);
	...
	private void toggleAccordion(String which) {
		...
		case "loai":
			...
			if (btnToggleLoai != null)
				btnToggleLoai.setText(loaiExpanded ? "▲" : "▼");
			break;
		case "dang":
			dangExpanded = !dangExpanded;
			pnlDangBody.setVisible(dangExpanded);
			if (btnToggleDang != null)
				btnToggleDang.setText(dangExpanded ? "▲" : "▼");
			break;
		case "nhom":
			nhomExpanded = !nhomExpanded;
			pnlNhomBody.setVisible(nhomExpanded);
			if (btnToggleNhom != null)
				btnToggleNhom.setText(nhomExpanded ? "▲" : "▼");
			break;
		}
	...
	private JPanel createCenterList() {
		...
		// Độ rộng cột: Mã, Mã vạch, Nhóm bệnh lý, Tên, Loại, Hoạt chất, Dạng, NSX, VAT,
		// Giá bán
		...
		stylePageBtn(btnPrev);
		btnNext = new JButton("Trang tiếp >");
		stylePageBtn(btnNext);
	...
	private void stylePageBtn(JButton btn) {
		...
		btn.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(COLOR_BORDER),
	...
	private void updatePagination() {
		...
		if (totalPages == 0)
			totalPages = 1;
		if (currentPageMock > totalPages)
			currentPageMock = totalPages;
		...
		for (int i = start; i < end; i++) {
			...
			// row: [0]=mã, [1]=tên, [2]=loại, [3]=hoạtChất, [4]=dạng, [5]=nsx, [6]=vat,
			// [7]=giaBan, [8]=maVach, [9]=nhomBenhLy
			...
			if (detailVisible) {
				...
				// Expanded: Mã, Mã vạch, Nhóm bệnh lý, Tên, Loại, Hoạt chất, Dạng, NSX, VAT,
				// Giá bán
				...
				modelSanPham.addRow(
						new Object[] { row[0], maVach, nhomBL, row[1], row[2], row[3], dang, nsx, vat, giaBan });
			}
		}
	}
	...
	private JPanel createRightDetail() {
		...
		g.gridx = 1;
		pnlForm.add(new JLabel("Mã vạch 1D (Barcode)") {
			{
				setFont(FONT_NORMAL);
			}
		}, g);
				...
				if (digits.length() == 13) {
					...
					if (fixed != null && !mv.equals(fixed)) {
						showCustomNotification("CHECK DIGIT SAI",
								"Ma vach " + mv + " co check digit sai! Da tu dong sua: " + fixed, "WARNING");
		...
		g.gridy = 4;
		g.gridwidth = 1;
		pnlForm.add(new JLabel("Tên viết tắt") {
			{
				setFont(FONT_NORMAL);
			}
		}, g);
		g.gridx = 1;
		pnlForm.add(new JLabel("Nhóm bệnh lý") {
			{
				setFont(FONT_NORMAL);
			}
		}, g);
		...
		cbNhomBenhLy.addActionListener(e -> {
			...
			if (sel != null && sel.toString().startsWith("+ Thêm nhóm")) {
				...
				if (tenMoi != null) {
					...
					if (!existsLieu)
						cbNhomBenhLieuNangCao.addItem(tenMoi);
				} else {
		...
		lblViTri.setFont(FONT_NORMAL);
		lblViTri.setForeground(Color.decode("#64748B"));
		pnlForm.add(lblViTri, g);
		g.gridy = 7;
		pnlViTriCard.setPreferredSize(new Dimension(0, 32));
		pnlForm.add(pnlViTriCard, g);
		g.gridwidth = 1;
		g.anchor = GridBagConstraints.CENTER; // reset

		// Row 6-7: Loại | Dạng bào chế
		cbLoaiCT = new JComboBox<>(
				new String[] { "Thuốc không kê đơn", "Thuốc kê đơn", "Sản phẩm chức năng", "Mỹ phẩm" });
		applyFlatComboBoxStyle(cbLoaiCT);
		cbDang = new JComboBox<>(new String[] { "Viên nén", "Viên nang", "Viên sủi", "Thuốc bột", "Kẹo ngậm",
				"Dung dịch", "Hỗn dịch", "Thuốc nhỏ giọt", "Súc miệng" });
		...
		pnlLoai = new JPanel(clLoai = new CardLayout());
		txtLoaiView = createViewField();
		pnlLoai.add(txtLoaiView, "VIEW");
		pnlLoai.add(cbLoaiCT, "EDIT");
		pnlDang = new JPanel(clDang = new CardLayout());
		txtDangView = createViewField();
		pnlDang.add(txtDangView, "VIEW");
		pnlDang.add(cbDang, "EDIT");
		pnlNSX = new JPanel(clNSX = new CardLayout());
		txtNSXView = createViewField();
		pnlNSX.add(txtNSXView, "VIEW");
		pnlNSX.add(cbNhaSX, "EDIT");
		pnlDVT = new JPanel(clDVT = new CardLayout());
		txtDVTView = createViewField();
		pnlDVT.add(txtDVTView, "VIEW");
		pnlDVT.add(cbDVT, "EDIT");
		...
		txtMoTa.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(COLOR_BORDER),
		...
		modelDonVi.addTableModelListener(e -> {
			...
			if (row < 0 || row >= modelDonVi.getRowCount())
				return;
		...
		lblDV.setOpaque(true);
		lblDV.setBackground(COLOR_HEADER_TABLE);
		lblDV.setForeground(Color.WHITE);
		lblDV.setFont(FONT_BOLD);
		...
		lblMV.setOpaque(true);
		lblMV.setBackground(COLOR_HEADER_TABLE);
		lblMV.setForeground(Color.WHITE);
		lblMV.setFont(FONT_BOLD);
		...
		txtScanBarcode.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(COLOR_BORDER),
		...
		for (int i = 0; i < tblLoHang.getColumnCount(); i++) {
			if (i == 3) {
				...
				tblLoHang.getColumnModel().getColumn(i).setCellRenderer(centerTbl);
			}
		}
		...
		lblLot.setBackground(COLOR_HEADER_TABLE);
		lblLot.setForeground(Color.WHITE);
		lblLot.setFont(FONT_BOLD);
		...
		btnCapNhatBottom = createBtnWithIcon("Cập nhật", "#1D68B2", new MenuIcon("EDIT"));
		btnCapNhatBottom.setPreferredSize(new Dimension(130, 36));
		btnXoaBottom = createBtnWithIcon("Ẩn sản phẩm", "#F59E0B", new MenuIcon("TRASH"));
		btnXoaBottom.setPreferredSize(new Dimension(140, 36));
		btnLuuBottom = createBtnWithIcon("Lưu", "#10B981", new MenuIcon("SAVE"));
		btnLuuBottom.setPreferredSize(new Dimension(100, 36));
		btnXacNhanThem = createBtnWithIcon("Xác nhận Thêm", "#E11D48", new MenuIcon("ADD"));
		btnXacNhanThem.setPreferredSize(new Dimension(170, 36));
		btnHuyBottom = createBtnWithIcon("Hủy", "#64748B", new MenuIcon("CANCEL"));
		btnHuyBottom.setPreferredSize(new Dimension(100, 36));
	...
	private void styleField(JTextField t) {
		...
		t.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(COLOR_BORDER),
				new EmptyBorder(0, 8, 0, 8)));
	...
	private void thucHienLuu() {
		...
		for (String[] mv : listMaVachData) {
			...
			if (mv.length > 2 && "LO".equals(mv[2]))
				continue;
			if (!codes.contains(mv[0]))
				codes.add(mv[0]);
		}
		...
		if (ten.isEmpty()) {
			...
			setErrorBorder(txtTen);
			if (firstError == null)
				firstError = txtTen;
		}
		if (vietTat.isEmpty()) {
			errors.add("Tên viết tắt");
			setErrorBorder(txtVietTat);
			if (firstError == null)
				firstError = txtVietTat;
		}
		if (hoatChat.isEmpty()) {
			errors.add("Hoạt chất");
			setErrorBorder(txtHoatChat);
			if (firstError == null)
				firstError = txtHoatChat;
		}
		if (hamLuong.isEmpty()) {
			errors.add("Hàm lượng");
			setErrorBorder(txtHamLuong);
			if (firstError == null)
				firstError = txtHamLuong;
		}
		if (vatStr.isEmpty()) {
			errors.add("Thuế VAT (%)");
		...
		if (giaBanStr.isEmpty()) {
			errors.add("Giá bán (VNĐ)");
			if (txtGiaBan != null)
				setErrorBorder(txtGiaBan);
			if (firstError == null)
				firstError = txtGiaBan;
		}
		if (!errors.isEmpty()) {
			...
			if (tenDVObj != null && !tenDVObj.toString().trim().isEmpty() && quyDoiDV != null && giaDVObj != null) {
				try {
					...
					if (tenDV.equalsIgnoreCase(dvtGoc)) {
						dsDonVi.add(new Object[] { tenDV, 1.0, gia });
						daCoDvtGoc = true;
		...
		txtNSXView.setText(nsx);
		txtDVTView.setText(dvt);
		txtNhomBenhLyView.setText(nhomBenhLy);
		txtViTriView.setText(cbViTri.getSelectedIndex() > 0 ? cbViTri.getSelectedItem().toString() : "");
		if (loai.equals("Thuốc kê đơn"))
			txtLoaiView.setDisabledTextColor(Color.decode("#EF4444"));
		else if (loai.equals("Sản phẩm chức năng"))
			txtLoaiView.setDisabledTextColor(Color.decode("#10B981"));
		else
			txtLoaiView.setDisabledTextColor(Color.decode("#2179E0"));
		...
		if (isAdding && bus.kiemTraMaSPTonTai(ma)) {
			showCustomNotification("TRÙNG MÃ HỆ THỐNG",
					"Mã sản phẩm " + ma
							+ " đã có người khác vừa dùng! Hệ thống đã tự lấy mã mới, vui lòng bấm Lưu lại.",
					"WARNING");
			txtId.setText(bus.taoMaMoi());
			return;
		}
		...
		boolean success;
		if (isAdding) {
			success = bus.themSP(ma, dbDanhMuc, dbDang, ten, vietTat, nsx, hoatChat, vat, hamLuong, moTa, dvt, giaBan,
					maVach, nhomBenhLy, viTriId, dsDonVi);
			if (success)
				showCustomNotification("THÊM MỚI THÀNH CÔNG", "Đã thêm sản phẩm thành công!", "SUCCESS");
		} else {
			success = bus.capNhatSP(ma, dbDanhMuc, dbDang, ten, vietTat, nsx, hoatChat, vat, hamLuong, moTa, dvt,
					giaBan, maVach, nhomBenhLy, viTriId, dsDonVi);
			if (success)
				showCustomNotification("CẬP NHẬT THÀNH CÔNG", "Cập nhật sản phẩm thành công!", "SUCCESS");
		}
	...
	private void hienThiChiTietSanPham(int row) {
		...
		txtMoTa.setText("");
		if (txtGiaBan != null)
			txtGiaBan.setText(data.length > 7 ? data[7].toString().replace(",", "") : "0");
		...
		if (spDayDu != null) {
			if (spDayDu.getHamLuong() != null)
				txtHamLuong.setText(spDayDu.getHamLuong());
			if (spDayDu.getMoTa() != null)
				txtMoTa.setText(spDayDu.getMoTa());
			if (spDayDu.getTenVietTat() != null)
				txtVietTat.setText(spDayDu.getTenVietTat());
			if (txtGiaBan != null) {
			...
			if (dsQuyDoi != null) {
				for (Object[] dv : dsQuyDoi) {
					...
					if (tiLeDb == 1.0 && giaBanGoc > 0) {
						giaHienThi = String.format("%,.0f", giaBanGoc);
					}
			...
			for (String mv : uniqueSpBarcodes) {
				String loaiMV = mv.equalsIgnoreCase(maChinh) ? phanLoaiMaVach(mv) + " ★" : phanLoaiMaVach(mv);
				listMaVachData.add(new String[] { mv, loaiMV }); // String[2] → không có flag → SP barcode
			}

			// 4. Bug1-fix: mã lô đưa vào listMaVachData với flag "LO" → readOnly, không lưu
			// vào SP
	...
	private static final String[] MENH_GIA_COLOR = { "#1565C0", "#AD1457", "#2E7D32", "#F9A825", "#C62828", "#6A1B9A",
			"#C62828", "#37474F", "#37474F" };

	private void showMenhGiaDialog() {
		if (txtGiaBan == null || !txtGiaBan.isEditable())
			return;
		...
		lblDisplay.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(Color.decode("#E2E8F0"), 1), new EmptyBorder(5, 15, 5, 15)));
		...
		for (int i = 0; i < MENH_GIA.length; i++) {
			...
			pnlCard.setBorder(BorderFactory.createCompoundBorder(
					BorderFactory.createLineBorder(Color.decode(colorHex), 1), new EmptyBorder(8, 10, 8, 10)));
		...
		btnXoaTrang.addActionListener(e -> {
			...
			for (Component c : pnlGrid.getComponents()) {
				if (c instanceof JPanel)
					((JLabel) ((JPanel) c).getComponent(1)).setText("0");
			}
		});
		...
		btnOK.setBackground(Color.decode("#1E3A8A"));
		...
		// 6. Tự động load giá hiện tại trên TextBox vào biến tổng tiền khi vừa mở
		// Dialog
		String cur = txtGiaBan.getText().trim().replace(",", "").replace(".", "");
		try {
			tongTien[0] = Long.parseLong(cur);
		} catch (Exception ignored) {
			tongTien[0] = 0;
		}
		lblDisplay.setText(String.format("%,.0f đ", (double) tongTien[0]).replace(",", "."));
	...
	private void setEditMode(boolean edit) {
		...
		txtTen.setBackground(bg);
		txtId.setEditable(false);
		txtId.setBackground(COLOR_DISABLED_BG);
		txtVietTat.setEditable(edit);
		txtVietTat.setBackground(bg);
		txtHoatChat.setEditable(edit);
		txtHoatChat.setBackground(bg);
		txtHamLuong.setEditable(edit);
		txtHamLuong.setBackground(bg);
		txtVAT.setEditable(edit);
		txtVAT.setBackground(bg);
		txtMaVach.setEditable(edit);
		txtMaVach.setBackground(bg);
		...
		if (edit) {
			if (isAdding)
				pnlActionBottom.add(btnXacNhanThem);
			else
				pnlActionBottom.add(btnLuuBottom);
		...
		pnlActionBottom.revalidate();
		pnlActionBottom.repaint();
	}
	...
	private void actionAnSanPham() {
		...
		String tenSP = txtTen.getText().trim();
		boolean confirmed = showCustomConfirmDialog("XÁC NHẬN ẨN SẢN PHẨM", "Sản phẩm <b>" + tenSP
				+ "</b> sẽ bị ẩn khỏi hệ thống.\nDữ liệu lịch sử vẫn được giữ nguyên.\n\nBạn có chắc chắn?");
		if (!confirmed)
			return;
		...
		if (soLuongTon > 0) {
			showCustomNotification("KHÔNG THỂ ẨN",
					"Sản phẩm còn " + soLuongTon + " đơn vị trong kho.\nXuất hết tồn kho trước khi ẩn.", "WARNING");
			return;
		}
		if (bus.anSP(maSP)) {
	...
	private void resetAllBorders() {
		resetBorder(txtTen);
		resetBorder(txtVietTat);
		resetBorder(txtHoatChat);
		resetBorder(txtHamLuong);
		resetBorder(txtVAT);
		if (txtGiaBan != null)
			resetBorder(txtGiaBan);
	}
	...
	private void setErrorBorder(JTextField t) {
		if (t != null)
			t.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.RED, 2),
					new EmptyBorder(0, 8, 0, 8)));
	}
	...
	private void setDetailVisible(boolean visible) {
		...
		if (visible) {
			if (pnlRightDetail.getParent() == null)
				pnlBody.add(pnlRightDetail, BorderLayout.EAST);
	...
	private void capNhatDuLieuComboBoxTuDB() {
		...
		cbNhaSX.addItem("DHG Pharma");
		cbNhaSX.addItem("Traphaco");
		for (String nsx : dsNSX) {
			boolean exists = false;
			for (int i = 0; i < cbNhaSX.getItemCount(); i++)
				if (cbNhaSX.getItemAt(i).equals(nsx))
					exists = true;
			if (!exists)
				cbNhaSX.addItem(nsx);
		}
		...
		for (String dvt : dsDVT) {
			...
			for (int i = 0; i < cbDVT.getItemCount(); i++)
				if (cbDVT.getItemAt(i).equals(dvt))
					exists = true;
			if (!exists)
				cbDVT.addItem(dvt);
		}
	...
	public void setReadOnly(boolean isReadOnly) {
		...
		if (isReadOnly) {
			if (topBtnThem != null)
				topBtnThem.setVisible(false);
			if (topBtnNhap != null)
				topBtnNhap.setVisible(false);
			if (topBtnXuat != null)
				topBtnXuat.setVisible(false);
		}
	}
	...
	private void applyTableCellRenderers() {
		...
		for (int i = 0; i < tblSanPham.getColumnCount(); i++) {
			...
			tblSanPham.getColumnModel().getColumn(i).setCellRenderer((i == 3 || i == 5 || i == 2) ? left : center);
		}
	}
	...
	class WatermarkTextField extends JTextField {
		...
		protected void paintComponent(Graphics g) {
			...
			if (getText().isEmpty() && !hasFocus()) {
				...
				g2.drawString(watermark, getInsets().left + 5, y);
				g2.dispose();
			}
		}
	}
	...
	private void applyFlatComboBoxStyle(JComboBox<String> cb) {
		...
		cb.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(COLOR_BORDER),
	...
	private void applyThinScrollBar(JScrollPane sp) {
		...
		sp.getVerticalScrollBar().setUI(new javax.swing.plaf.basic.BasicScrollBarUI() {
			...
			protected void configureScrollBarColors() {
				...
				trackColor = Color.decode("#F1F5F9");
			}
	...
	private boolean showCustomConfirmDialog(String titleText, String message) {
		...
		btnYes.setBackground(Color.decode("#1E3A8A"));
		...
		btnNo.setBackground(Color.decode("#64748B"));
	...
	private static class ToggleEditableTableModel extends DefaultTableModel {
		...
		public ToggleEditableTableModel(Object[] cols, int rows) {
			super(cols, rows);
		}
	...
	private void thucHienXuatExcelThang() {
		...
		java.awt.FileDialog fd = new java.awt.FileDialog(parentFrame, "Chọn nơi lưu file Excel",
		...
		if (dir != null && file != null) {
			...
			if (fullData == null) {
				...
				showCustomNotification("Lỗi", "Không thể lấy dữ liệu!", "ERROR");
				return;
			}
			String result = exportExcelAction(filePath, fullData);
			setCursor(Cursor.getDefaultCursor());
			if (result.equals("SUCCESS"))
				showCustomNotification("XUẤT THÀNH CÔNG", "Đã xuất " + fullData.size() + " sản phẩm:\n" + filePath,
						"SUCCESS");
			else if (result.equals("FILE_OPEN"))
				showCustomNotification("LỖI GHI ĐÈ", "File đang mở! Đóng file Excel trước.", "ERROR");
			else
				showCustomNotification("LỖI", "Xuất thất bại: " + result, "ERROR");
		}
	}
	...
	private String exportExcelAction(String path, List<SanPham> data) {
		try (Workbook workbook = new XSSFWorkbook()) {
			...
			String[] headers = { "id", "danhMuc", "dang", "ten", "tenVietTat", "nhaSanXuat", "hoatChat", "thueVAT",
					"hamLuong", "moTa", "donViDoCoBan", "ngayTao", "trangThai", "giaBan", "maVach", "nhomBenhLy" };
			...
			for (int i = 0; i < data.size(); i++) {
				...
				row.createCell(1)
						.setCellValue(sp.getDanhMuc() != null ? mapDanhMucToExcelLabel(sp.getDanhMuc().name()) : "");
				...
				row.createCell(7)
						.setCellValue(Double.parseDouble(String.format(java.util.Locale.US, "%.2f", sp.getThueVAT())));
				row.createCell(8).setCellValue(
						(sp.getHamLuong() != null && !sp.getHamLuong().isEmpty()) ? sp.getHamLuong() : "NULL");
				row.createCell(9)
						.setCellValue((sp.getMoTa() != null && !sp.getMoTa().isEmpty()) ? sp.getMoTa() : "NULL");
	...
	private void thucHienNhapExcelThang() {
		Frame parentFrame = (Frame) SwingUtilities.getWindowAncestor(this);
		java.awt.FileDialog fd = new java.awt.FileDialog(parentFrame, "Chọn file Excel để nhập",
		...
		if (dir != null && file != null) {
			...
			if (!fileToImport.exists()) {
				showCustomNotification("LỖI FILE", "Không tìm thấy file!", "ERROR");
				return;
			}
			setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
			try (java.io.FileInputStream fis = new java.io.FileInputStream(fileToImport);
				...
				for (int i = 1; i <= sheet.getLastRowNum(); i++) {
					...
					try {
						...
						for (SanPham sp : dsHienTai) {
							if (sp.getId().equalsIgnoreCase(idTuFile)) {
								spCu = sp;
								break;
							}
						}
						...
						if (spCu != null) {
							...
							try {
								...
								if (cellGia != null) {
									if (cellGia.getCellType() == org.apache.poi.ss.usermodel.CellType.NUMERIC) {
										...
										if (!gs.isEmpty() && !gs.equals("NULL"))
											giaBanExcel = Double.parseDouble(gs);
									}
								}
							} catch (Exception ignored) {
							}
							...
							if (dsDonViCu == null)
								dsDonViCu = new ArrayList<>();
							boolean updated = bus.capNhatSP(spCu.getId(), mapToDbDanhMuc(loai), mapToDbDang(dang), ten,
									vietTat, nsx, hoatChat, vat, hamLuong, moTa, donVi, giaBanExcel, maVach, nhomBL,
									viTriCu, dsDonViCu);
							...
							try {
								...
								if (cellGia != null) {
									if (cellGia.getCellType() == org.apache.poi.ss.usermodel.CellType.NUMERIC) {
										...
										String gs = formatter.formatCellValue(cellGia).replace(",", "").trim();
										if (!gs.isEmpty() && !gs.equals("NULL"))
											giaBanMoi = Double.parseDouble(gs);
									}
								}
							} catch (Exception ignored) {
							}
							...
							if (maVach == null || maVach.isEmpty()) {
								...
								try {
									...
									String digits12 = String.format("200%09d",
							...
							// 6. Lưu xuống DB
							boolean isSaved = bus.themSP(newId, mapToDbDanhMuc(loai), mapToDbDang(dang), ten, vietTat,
									nsx, hoatChat, vat, hamLuong, moTa, donVi, giaBanMoi, maVach, nhomBL, "",
									new ArrayList<>());
				...
				if (!missingDataRows.isEmpty()) {
					msg.append("\n⚠ Bỏ qua dòng thiếu dữ liệu: ").append(missingDataRows);
					hasError = true;
				}
				if (!errorRows.isEmpty()) {
					msg.append("\n⚠ Lỗi format dòng: ").append(errorRows);
					hasError = true;
				}
				showCustomNotification(hasError ? "NHẬP CÓ CẢNH BÁO" : "NHẬP THÀNH CÔNG", msg.toString(),
	...
	/**
	 * Đồng bộ giá bán vào bảng đơn vị: - Cập nhật dòng ĐVT gốc (tỷ lệ = 1) theo
	 * txtGiaBan - Tự động tính giá các dòng khác = giaBanGoc × tỷ lệ
	 */
	...
	private void syncGiaBanVaoBangDonVi() {
		...
		if (gs.isEmpty())
			return;
		try {
			giaBanGoc = Double.parseDouble(gs);
		} catch (Exception e) {
			return;
		}
		if (giaBanGoc <= 0)
		...
		for (int i = 0; i < modelDonVi.getRowCount(); i++) {
			...
			Object tiLeObj = modelDonVi.getValueAt(i, 1);
			if (tenObj == null || tenObj.toString().trim().isEmpty())
				continue;
			String ten = tenObj.toString().trim();
			double tiLe = 1.0;
			try {
				tiLe = Double.parseDouble(tiLeObj.toString());
			} catch (Exception ex) {
			}
	...
	/**
	 * Phân loại mã vạch đúng chuẩn GS1. - Prefix 200-299: mã nội bộ nhà thuốc (tự
	 * sinh, không đăng ký GS1) - Prefix khác + 13 chữ số: EAN-13 thật của nhà sản
	 * xuất - 8 chữ số: EAN-8 (GS1) - Có chữ hoặc gạch: Code 128 nội bộ (mã lô, mã
	 * SP)
	 */
	private String phanLoaiMaVach(String mv) {
		...
		if (mv.equals(digits)) {
			if (mv.length() == 13) {
				...
				if (prefix3 >= 200 && prefix3 <= 299)
					return "Nội bộ EAN-13";
				return "EAN-13 (GS1)";
			}
			if (mv.length() == 8)
				return "EAN-8 (GS1)";
	...
	/**
	 * Tinh check digit EAN-13 theo chuan GS1. Input: 12 chu so dau. Output: check
	 * digit (0-9).
	 */
	...
	/**
	 * Chuan hoa ma vach EAN-13: giu 12 digits dau, tinh lai check digit. Tra ve
	 * null neu khong phai dinh dang EAN (12 hoac 13 chu so).
	 */
	...
	private void thucHienInMaVach1D(String barcodeData, String productName, String price) {
		...
		try {
			...
			barcodeImg = MatrixToImageWriter
					.toBufferedImage(writer.encode(barcodeData, BarcodeFormat.CODE_128, 520, 150));
		} catch (Exception ex) {
		...
		JDialog dlg = new JDialog((java.awt.Frame) SwingUtilities.getWindowAncestor(this), "In / Xuất mã vạch sản phẩm",
				true);
		...
		JPanel pnlImg = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				int margin = 8;
				g2.drawImage(finalImg, margin, margin, getWidth() - margin * 2, getHeight() - margin * 2, null);
			}
		};
		...
		btnXuatAnh.addActionListener(ev -> {
			...
			java.awt.FileDialog fd = new java.awt.FileDialog(parentFrame, "Chọn nơi lưu ảnh mã tem dán",
					java.awt.FileDialog.SAVE);
			...
			if (dir != null && file != null) {
				...
				try {
					...
					java.awt.image.BufferedImage labelImg = new java.awt.image.BufferedImage(540, 240,
	...
	private JPanel createMaVachCard(String barcode, String loai, int index, boolean readOnly) {
		...
		card.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(readOnly ? Color.decode("#FDE68A") : Color.decode("#E2E8F0"), 1),
		...
		if (readOnly) {
			...
			lblBadge.setBackground(Color.decode("#FEF9C3"));
			lblBadge.setForeground(Color.decode("#854D0E"));
		} else if (loai.contains("★")) {
			lblBadge.setBackground(Color.decode("#DBEAFE"));
			lblBadge.setForeground(Color.decode("#1D4ED8"));
		} else if (loai.contains("GS1") || loai.contains("UPC") || loai.contains("GTIN")) {
			lblBadge.setBackground(Color.decode("#DCFCE7"));
			lblBadge.setForeground(Color.decode("#15803D"));
		} else {
			lblBadge.setBackground(Color.decode("#F0F9FF"));
			lblBadge.setForeground(Color.decode("#0369A1"));
		}
	...
	/**
	 * @param boQuaLoc true = load tất cả không lọc (dùng sau khi lưu/xóa) false =
	 *                 lọc theo nhóm bệnh đang chọn
	 */
	private void loadDanhSachComboNangCaoRaGiaoDien(boolean boQuaLoc) {
		...
		String nhomLoc = "";
		if (!boQuaLoc && cbNhomBenhLieuNangCao != null && cbNhomBenhLieuNangCao.getSelectedItem() != null) {
			nhomLoc = cbNhomBenhLieuNangCao.getSelectedItem().toString().trim();
		}

		for (Object[] r : bus.layDanhSachComboNangCao()) {
			if (nhomLoc.isEmpty() || nhomLoc.startsWith("--") || r[2].toString().equalsIgnoreCase(nhomLoc)) {
				mdlDanhSachLieuNangCao
						.addRow(new Object[] { r[0], r[1], r[2], String.format("%,.0f đ", (Double) r[3]) });
			}
		}
		dangCapNhatBangLieu = false;
	}
	...
	private void xuLyQuetBarcodeNhanhLieu() {
		...
		boolean coTheLaBarcode = code.length() >= 8 && code.matches("[A-Za-z0-9\\-\\.]+");
	...
	private void scheduleLieuAuto() {
		autocompleteTimerLieu.stop();
		String text = txtSearchLieuNangCao.getText();
		if (text != null && text.trim().length() >= 2)
			autocompleteTimerLieu.start();
	}
	...
	private void thucHienAutocompleteLieu() {
		...
		if (res.isEmpty()) {
			...
			int popupHeight = 32;
			popupTimThuoc.setPreferredSize(new Dimension(txtSearchLieuNangCao.getWidth() + 200, popupHeight));
			popupTimThuoc.show(txtSearchLieuNangCao, 0, txtSearchLieuNangCao.getHeight());
			listPopupThuoc.clearSelection(); // không cho chọn
		} else {
			for (Object[] r : res) {
				modelPopupThuoc.addElement(String.format("%s | %s | Giá: %,.0f đ", r[1], r[2], (Double) r[3]));
			}
	...
	private void themThuocVaoDocLieu() {
		...
		for (SanPham.ChiTietLieu ct : dsChiTietLieuLuuTru) {
			if (ct.getSanPhamId().equals(maSPDangChonLieu)) {
				showCustomNotification("TRÙNG THUỐC", tenSPDangChonLieu + " đã có trong toa rồi!", "WARNING");
				return;
			}
		}
		...
		try {
			...
			SanPham.ChiTietLieu ct = new SanPham.ChiTietLieu(comboIdDangSua, maSPDangChonLieu, tenSPDangChonLieu,
			...
			if (ct.getTongSoLuong() > tonKho) {
				if (!showCustomConfirm("VƯỢT TỒN KHO",
						"<b>" + tenSPDangChonLieu + "</b> chỉ còn <b>" + tonKho + "</b> " + dvtDangChonLieu
								+ " trong kho.<br>" + "Toa yêu cầu <b>" + ct.getTongSoLuong() + "</b> "
								+ dvtDangChonLieu + ".<br><br>" + "Vẫn tiếp tục thêm vào toa?"))
					return;
			}
			...
			showCustomNotification("SAI ĐỊNH DẠNG", "Vui lòng nhập số hợp lệ cho liều lượng và số ngày!", "ERROR");
		}
	}
	...
	private void HienThiDongChiTietMoiLenBang(SanPham.ChiTietLieu ct) {
		...
		dangCapNhatBangLieu = true;
		modChiTietLieu.addRow(new Object[] { ct.getSanPhamId(), ct.getTenSanPham(), ct.getDvt(), ct.getSang(),
				ct.getTrua(), ct.getChieu(), ct.getToi(), ct.getCachDung(), ct.getSoNgay(), ct.getTongSoLuong(),
				String.format("%,.0f", ct.tinhThanhTien()), "Xóa" });
	...
	private void onTableCellLieuChanged(javax.swing.event.TableModelEvent e) {
		...
		try {
			...
			case COL_L_SANG: {
				...
				if (v < 0)
					throw new NumberFormatException("Liều Sáng < 0");
				ct.setSang(v);
				break;
			}
			case COL_L_TRUA: {
				double v = Double.parseDouble(modChiTietLieu.getValueAt(row, col).toString());
				if (v < 0)
					throw new NumberFormatException("Liều Trưa < 0");
				ct.setTrua(v);
				break;
			}
			case COL_L_CHIEU: {
				double v = Double.parseDouble(modChiTietLieu.getValueAt(row, col).toString());
				if (v < 0)
					throw new NumberFormatException("Liều Chiều < 0");
				ct.setChieu(v);
				break;
			}
			case COL_L_TOI: {
				double v = Double.parseDouble(modChiTietLieu.getValueAt(row, col).toString());
				if (v < 0)
					throw new NumberFormatException("Liều Tối < 0");
				ct.setToi(v);
				break;
			}
			case COL_L_NGAY: {
				...
				if (n <= 0)
					throw new NumberFormatException("Số ngày phải >= 1");
				ct.setSoNgay(n);
				break;
			}
			}
			...
			if (ct.getTongSoLuong() > tonKho) {
				SwingUtilities.invokeLater(() -> showCustomNotification("VƯỢT TỒN KHO",
						ct.getTenSanPham() + " chỉ còn " + tonKho + " " + ct.getDvt()
								+ " trong kho, nhưng toa đang yêu cầu " + ct.getTongSoLuong() + " " + ct.getDvt() + ".",
						"WARNING"));
			}
		} catch (Exception ignored) {
			...
			switch (col) {
			case COL_L_SANG:
				modChiTietLieu.setValueAt(ct.getSang(), row, col);
				break;
			case COL_L_TRUA:
				modChiTietLieu.setValueAt(ct.getTrua(), row, col);
				break;
			case COL_L_CHIEU:
				modChiTietLieu.setValueAt(ct.getChieu(), row, col);
				break;
			case COL_L_TOI:
				modChiTietLieu.setValueAt(ct.getToi(), row, col);
				break;
			case COL_L_NGAY:
				modChiTietLieu.setValueAt(ct.getSoNgay(), row, col);
				break;
			}
		} finally {
			dangCapNhatBangLieu = false;
	...
	private void resetFormSoanThaoLieu() {
		comboIdDangSua = null;
		dsChiTietLieuLuuTru.clear();
		modChiTietLieu.setRowCount(0);
		txtTenComboNangCao.setText("");
		txtGhiChuLieuNangCao.setText("");
		txtGiaBanComboNangCao.setValue(0L);
		lblComboIdNangCao.setText("(Tự động sinh)");
		lblTongTienLieuNangCao.setText("0 đ");
	...
	private void onChonMauLieuTuDanhSach() {
		...
		BUS_SanPham bus = new BUS_SanPham();
		SanPham.MauLieu m = bus.layComboByIdNangCao(id);
		if (m == null)
			return;

		dangCapNhatBangLieu = true; // Khóa giữ trạng thái, không cho phép gãy dòng dữ liệu
		comboIdDangSua = m.getComboId();
		txtTenComboNangCao.setText(m.getTenCombo());
		cbNhomBenhLieuNangCao.setSelectedItem(m.getNhomBenh());
		txtGiaBanComboNangCao.setValue((long) m.getGiaBanCombo());
		txtGhiChuLieuNangCao.setText(m.getGhiChu());
		lblComboIdNangCao.setText(m.getComboId());
		dsChiTietLieuLuuTru.clear();
		modChiTietLieu.setRowCount(0);
		...
		for (SanPham.ChiTietLieu ct : m.getDsChiTiet()) {
			...
			if (uomMin != null) {
				...
				if (spMoi != null)
					ct.setGiaDonVi(spMoi.getGiaBan());
			}
			dsChiTietLieuLuuTru.add(ct);
			HienThiDongChiTietMoiLenBang(ct);
		}
	...
	private void luuMauLieuChuyenSau() {
		...
		if (ten.isEmpty()) {
			showCustomNotification("THIẾU TÊN MẪU", "Vui lòng nhập tên cho mẫu thuốc phối liều trước khi lưu.",
					"WARNING");
			txtTenComboNangCao.requestFocus();
			return;
		}
		if (dsChiTietLieuLuuTru.isEmpty()) {
			showCustomNotification("TOA TRỐNG", "Vui lòng tìm và thêm ít nhất 1 loại thuốc vào toa trước khi lưu.",
					"WARNING");
			txtSearchLieuNangCao.requestFocus();
			return;
		}
		...
		if (comboIdDangSua == null) {
			...
			if (bus.kiemTraTenComboTonTai(ten)) {
				showCustomNotification("TRÙNG TÊN MẪU", "Đã tồn tại mẫu liều có tên \"" + ten + "\".\n"
						+ "Vui lòng đặt tên khác hoặc mở mẫu đó để chỉnh sửa.", "WARNING");
				txtTenComboNangCao.requestFocus();
				return;
			}
		...
		if (ok) {
			...
			SwingUtilities.invokeLater(() -> {
				...
				if (lastRow >= 0) {
					tblDanhSachLieuNangCao.scrollRectToVisible(tblDanhSachLieuNangCao.getCellRect(lastRow, 0, true));
	...
	private void xoaMauLieuHienTai() {
		...
		if (showCustomConfirm("XÁC NHẬN XÓA", "Bạn chắc chắn muốn xóa mẫu liều:<br><b>" + tenMau + "</b>?<br>"
	...
	private boolean showCustomConfirm(String title, String message) {
		...
		pnl.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.decode("#F59E0B"), 2),
	...
	private String showNhomBenhLyInputDialog(JComboBox<String> existingCombo) {
		...
		JDialog dialog = new JDialog((java.awt.Frame) SwingUtilities.getWindowAncestor(this), true);
		dialog.setUndecorated(true);
		...
		// Mô tả nhỏ
		JLabel lblDesc = new JLabel("<html><span style='color:#64748B;font-size:12px'>"
		...
		txtNhomMoi.setBorder(
				BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.decode("#CBD5E1"), 1, true),
		...
		pnlHint.setBorder(
				BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.decode("#E2E8F0"), 1, true),
		...
		btnHuy.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				btnHuy.setBackground(Color.decode("#475569"));
			}
		...
		btnThem.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				btnThem.setBackground(Color.decode("#1e40af"));
			}
