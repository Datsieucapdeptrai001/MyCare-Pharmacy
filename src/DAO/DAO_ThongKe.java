...
public class DAO_ThongKe {
    ...
    public double tinhDoanhThu(LocalDateTime tuNgay, LocalDateTime denNgay) {
        double doanhThu = 0;
        String sql = "SELECT SUM(CASE WHEN ct.donGiaThucTe > 0 THEN ct.thanhTien ELSE (ct.soLuong*dvl.gia) END) / 1.1 AS DoanhThu "
                +
                "FROM HoaDon hd " +
    ...
    public double[] getDoanhThu12Thang(int year, String filter) {
        ...
        if (filter != null && !filter.isEmpty()) {
            if (filter.trim().startsWith("AND"))
                condHD = " " + filter;
            else
                condHD = " AND hd.nhanVienId='" + filter + "'";
        }
    ...
    public double[] getChiPhi12Thang(int year, String filter) {
        double[] data = new double[12];
        String condHD = (filter != null && !filter.isEmpty())
                ? (filter.trim().startsWith("AND") ? filter : " AND hd.nhanVienId='" + filter + "'")
                : "";
     ...
     * Mỗi Object[4]: {tenSP(String), danhMuc(String), soLuong(int),
     * doanhThu_triệu(double)}
     */
    public List<Object[]> getTopSanPham(int year, String condHD) {
        List<Object[]> result = new ArrayList<>();
        String sql = "SELECT TOP 10 sp.ten, sp.danhMuc, SUM(ct.soLuong) sl, " +
                "ISNULL(SUM(CASE WHEN ct.donGiaThucTe > 0 THEN ct.thanhTien ELSE (ct.soLuong*dvl.gia) END) / 1.1,0) dt " +
     ...
     * Trả về Object[4]: {tongHD(int), tongDT_trieu(double), tongKH(int),
     * tongSPBan(int)}
     */
    public Object[] getThongKeNgayCuThe(String dateYMD) {
        String sql = "SELECT COUNT(DISTINCT hd.id) tongHD, " +
                "ISNULL(SUM(CASE WHEN ct.donGiaThucTe > 0 THEN ct.thanhTien ELSE (ct.soLuong*dvl.gia) END)/1.1,0)/1000000.0 tongDT, " +
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next())
                return new Object[] { rs.getInt("tongHD"), rs.getDouble("tongDT"), rs.getInt("tongKH"),
                        rs.getInt("tongSP") };
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * Logic: Duyệt từng HĐ → apply KM qua tinhTienThucTeCuaHoaDon → phân bổ tỉ lệ cho từng SP → chia 1.1 → tổng hợp
     */
    public List<Object[]> getTopSPTrongNgay(String dateYMD) {
        // Bước 1: Lấy danh sách HĐ + ghiChu + tổng gốc có VAT
        java.util.Map<String, double[]> spMap = new java.util.LinkedHashMap<>(); // tenSP -> {soLuong, doanhThu}
        ...
        try (PreparedStatement ps = con.prepareStatement(sqlHD)) {
            ...
            while (rs.next()) {
                ...
                // Bước 2: Lấy chi tiết SP của HĐ này
                ...
                try (PreparedStatement ps2 = con.prepareStatement(sqlCT)) {
                    ...
                    try (ResultSet rs2 = ps2.executeQuery()) {
                        while (rs2.next()) {
                            ...
                            double dtSP = (gocSP * tyLe) / 1.1; // Phân bổ KM + trừ VAT
        ...
        // Bước 3: Sắp xếp theo SL giảm dần, lấy top 10, convert sang triệu
        List<Object[]> result = new ArrayList<>();
     ...
     * Logic: Duyệt từng HĐ → apply KM → chia 1.1 → gộp theo giờ
     */
    ...
    public List<Object[]> getNVTrongNgay(String dateYMD) {
        ...
        String sql = "SELECT nv.hoVaTen, COUNT(DISTINCT hd.id) soHD, " +
                "ISNULL(SUM(CASE WHEN ct.donGiaThucTe > 0 THEN ct.thanhTien ELSE (ct.soLuong*dvl.gia) END)/1.1,0)/1000000.0 dt " +
                "FROM HoaDon hd " +
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next())
                result.add(new Object[] { rs.getString("hoVaTen"), rs.getInt("soHD"), rs.getDouble("dt") });
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    public List<Object[]> getThongKeTuan(String weekStartYMD) {
        List<Object[]> result = new ArrayList<>();
        String sql = "SELECT CONVERT(NVARCHAR,CAST(hd.ngayLapHD AS DATE),103) d, " +
                "COUNT(DISTINCT hd.id) cnt, ISNULL(SUM(CASE WHEN ct.donGiaThucTe > 0 THEN ct.thanhTien ELSE (ct.soLuong*dvl.gia) END)/1.1,0)/1000000.0 dt " +
                "FROM HoaDon hd " +
                "JOIN ChiTietHoaDon ct ON ct.hoaDonId=hd.id " +
                "JOIN DonViDoLuong dvl ON dvl.id=ct.donViDoLuongId AND dvl.sanPhamId=ct.sanPhamId " +
                "WHERE hd.loaiHD='BAN_HANG' " +
     ...
     * Logic: Duyệt từng HĐ → apply KM → chia 1.1 → gộp theo KH
     */
    public List<Object[]> getTopKhachHang(int year, String condHD) {
        // Map: hoVaTen -> {soHD, tongDT, diemTichLuy}
        java.util.Map<String, double[]> khMap = new java.util.LinkedHashMap<>();
        String sql = "SELECT kh.hoVaTen, kh.diemTichLuy, hd.id, hd.ghiChu, " +
                "SUM(ct.soLuong * dvl.gia * (1 + ISNULL(sp.thueVAT, 0)/100.0)) as tongGocCoVAT " +
                "FROM KhachHang kh " +
                ...
                " GROUP BY kh.hoVaTen, kh.diemTichLuy, hd.id, hd.ghiChu";
        Connection con = getConn();
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, year);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ...
                double tongGoc = rs.getDouble("tongGocCoVAT");
                double thucTe = tinhTienThucTeCuaHoaDon(con, tongGoc, rs.getString("ghiChu"));
                double dtThuan = (thucTe / 1.1) / 1_000_000.0;
    ...
    /**
     * Top SP tồn nhiều nhất. Mỗi Object[3]: {tenSP, soLuongTon(int),
     * giaTriTrieu(double)}
     */
    ...
    public int getHoaDonHomNay(String maNV) {
        ...
        String sql = "SELECT COUNT(*) FROM HoaDon WHERE CONVERT(DATE,ngayLapHD)=CONVERT(DATE,GETDATE()) AND loaiHD='BAN_HANG'"
                + cond;
        try (Statement st = getConn().createStatement(); ResultSet rs = st.executeQuery(sql)) {
            if (rs.next())
                count = rs.getInt(1);
    ...
    public double getDoanhThuTienMatHomNay(String maNV) {
        ...
        String sql = "SELECT hd.ghiChu, " +
                ...
                "JOIN DonViDoLuong dvl ON ct.donViDoLuongId=dvl.id AND ct.sanPhamId=dvl.sanPhamId " +
                "JOIN SanPham sp ON ct.sanPhamId = sp.id " +
                "WHERE hd.phuongThucThanhToan='TIEN_MAT' AND CONVERT(DATE,hd.ngayLapHD)=CONVERT(DATE,GETDATE()) AND hd.loaiHD='BAN_HANG'"
                + cond +
                " GROUP BY hd.id, hd.ghiChu";
        try (Connection con = getConn(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) {
    ...
    public int getSoLoHangSapHetHanKhoang(int days) {
        int count = 0;
        String sql = "SELECT COUNT(*) FROM LoHang WHERE ngayHetHan IS NOT NULL AND DATEDIFF(DAY,GETDATE(),ngayHetHan)<="
                + days;
        try (Statement st = getConn().createStatement(); ResultSet rs = st.executeQuery(sql)) {
            if (rs.next())
                count = rs.getInt(1);
    ...
    public List<Object[]> getLoHangSapHetHanNhanh(int days) {
        List<Object[]> list = new ArrayList<>();
        String sql = "SELECT TOP 8 sp.ten, lh.soLoHang, lh.soLuongLoHang, lh.ngayHetHan, DATEDIFF(DAY,GETDATE(),lh.ngayHetHan) cl FROM LoHang lh JOIN SanPham sp ON lh.sanPhamId=sp.id WHERE lh.ngayHetHan IS NOT NULL AND DATEDIFF(DAY,GETDATE(),lh.ngayHetHan)<="
                + days + " ORDER BY lh.ngayHetHan ASC";
        try (Statement st = getConn().createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next())
                list.add(new Object[] { rs.getString("ten"), rs.getString("soLoHang"), rs.getInt("soLuongLoHang"),
                        rs.getTimestamp("ngayHetHan"), rs.getInt("cl") });
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return list;
    ...
    private String getCaCondition(int ca) {
        String cond = "";
        if (ca == 1)
            cond = " AND DATEPART(HOUR, hd.ngayLapHD) BETWEEN 6 AND 13";
        else if (ca == 2)
            cond = " AND DATEPART(HOUR, hd.ngayLapHD) BETWEEN 14 AND 21";
        else if (ca == 3)
            cond = " AND ((DATEPART(HOUR, hd.ngayLapHD) >= 22) OR (DATEPART(HOUR, hd.ngayLapHD) < 6))";
        ...
        try {
            ...
            if (session != null && !session.isAdmin() && session.getCaHienTai() != null) {
                ...
                // FIX LỖI: Chuyển sang định dạng ISO 8601 (có chữ 'T') để SQL Server phân tích
                // chính xác tuyệt đối
    ...
    public int getHoaDonHomNay(String maNV, int ca) {
        ...
        String sql = "SELECT COUNT(*) FROM HoaDon hd WHERE CONVERT(DATE, hd.ngayLapHD)=CONVERT(DATE, GETDATE()) AND hd.loaiHD='BAN_HANG'"
                + cond;
        try (Statement st = getConn().createStatement(); ResultSet rs = st.executeQuery(sql)) {
            if (rs.next())
                count = rs.getInt(1);
    ...
    public double getTongTienHangHomNay(String maNV, int ca) {
        ...
        String cond = (maNV != null && !maNV.isEmpty()) ? " AND hd.nhanVienId='" + maNV + "'" : "";
        cond += getCaCondition(ca);
        String sql = "SELECT ISNULL(SUM(ct.soLuong * dvl.gia),0) FROM HoaDon hd JOIN ChiTietHoaDon ct ON hd.id = ct.hoaDonId JOIN DonViDoLuong dvl ON ct.donViDoLuongId = dvl.id AND ct.sanPhamId = dvl.sanPhamId WHERE CONVERT(DATE, hd.ngayLapHD) = CONVERT(DATE, GETDATE()) AND hd.loaiHD = 'BAN_HANG' "
                + cond;
        try (Connection con = getConn(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            if (rs.next())
                tong = rs.getDouble(1);
    ...
    // 2. Hàm lấy TỔNG KHUYẾN MÃI (Đã Fix: Tính qua chênh lệch giá gốc và thanh toán
    // thực tế)
    ...
    public double getDoanhThuHomNay(String maNV, int ca) {
        ...
        String cond = (maNV != null && !maNV.isEmpty()) ? " AND hd.nhanVienId='" + maNV + "'" : "";
        cond += getCaCondition(ca);
        String sql = "SELECT hd.ghiChu, SUM(ct.soLuong * dvl.gia * (1 + ISNULL(sp.thueVAT, 0)/100.0)) as tongGoc FROM HoaDon hd JOIN ChiTietHoaDon ct ON hd.id=ct.hoaDonId JOIN DonViDoLuong dvl ON ct.donViDoLuongId=dvl.id AND ct.sanPhamId=dvl.sanPhamId JOIN SanPham sp ON ct.sanPhamId = sp.id WHERE CONVERT(DATE, hd.ngayLapHD) = CONVERT(DATE, GETDATE()) AND hd.loaiHD = 'BAN_HANG'"
                + cond + " GROUP BY hd.id, hd.ghiChu";
        try (Connection con = getConn(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next())
                dt += tinhTienThucTeCuaHoaDon(con, rs.getDouble("tongGoc"), rs.getString("ghiChu"));
        } catch (Exception e) {
    ...
    public double getDoanhThuTienMatHomNay(String maNV, int ca) {
        ...
        String cond = (maNV != null && !maNV.isEmpty()) ? " AND hd.nhanVienId='" + maNV + "'" : "";
        cond += getCaCondition(ca);
        String sql = "SELECT hd.ghiChu, SUM(ct.soLuong * dvl.gia * (1 + ISNULL(sp.thueVAT, 0)/100.0)) as tongGoc FROM HoaDon hd JOIN ChiTietHoaDon ct ON hd.id=ct.hoaDonId JOIN DonViDoLuong dvl ON ct.donViDoLuongId=dvl.id AND ct.sanPhamId=dvl.sanPhamId JOIN SanPham sp ON ct.sanPhamId = sp.id WHERE hd.phuongThucThanhToan = 'TIEN_MAT' AND CONVERT(DATE, hd.ngayLapHD) = CONVERT(DATE, GETDATE()) AND hd.loaiHD = 'BAN_HANG'"
                + cond + " GROUP BY hd.id, hd.ghiChu";
        try (Connection con = getConn(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next())
                dt += tinhTienThucTeCuaHoaDon(con, rs.getDouble("tongGoc"), rs.getString("ghiChu"));
    ...
    public double getTienHoanTraTheoCa(String maNV, LocalDateTime start) {
        ...
        // ĐÃ FIX: Chỉ tính tiền chi ra đối với những phiếu đã "Hoàn thành" (Đã tiếp
        // nhận)
        ...
        try (Connection con = getConn(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    // Bóc tách số tiền trực tiếp từ chuỗi Ghi chú giống như trên giao diện
                    // ManHinhDoiTra
                    if (ghiChu != null && ghiChu.contains("|")) {
                        ...
                        if (loai.equals("TRA_HANG")) {
                            ...
                            if (parts.length >= 4) {
                                ...
                                // Đổi hàng chỉ tính là "Chi ra" khi Cửa hàng phải thối lại tiền cho khách
                                // (Chênh lệch âm / Hoàn tiền)
    ...
    public List<Object[]> getGoiYKhuyenMai(int year) {
        List<Object[]> list = new ArrayList<>();
        String sql = "SELECT TOP 10 sp.tenSanPham, dm.tenDanhMuc, " +
                "SUM(ct.soLuong) AS soLuongBan, " +
                "SUM(ct.soLuong * dvl.gia) AS doanhThu, " +
                "MAX(sp.giaNhap) AS giaVon " +
                "FROM HoaDon hd " +
                "JOIN ChiTietHoaDon ct ON hd.id = ct.hoaDonId " +
                "JOIN DonViDoLuong dvl ON ct.donViDoLuongId = dvl.id AND ct.sanPhamId = dvl.sanPhamId " +
                "JOIN SanPham sp ON ct.sanPhamId = sp.id " +
                ...
                "GROUP BY sp.tenSanPham, dm.tenDanhMuc " +
        ...
        try (Connection con = getConn(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    list.add(new Object[] { tenSP, danhMuc, slBan, dtTrieu, giaVon, bienLNPct, coKM, goiYLoai, lyDo,
                            mucGiam });
                }
            }
        } catch (Exception e) {
    ...
    /**
     * Hóa đơn giá trị cao hôm nay (> 1 triệu), tối đa 5 dòng.
     * Object[4]: {maHD, tenKH, thanhTien(double), gio(String)}
     */
    ...
    /**
     * Khách hàng VIP (điểm >= 500) mua hàng gần đây nhất hôm nay.
     * Object[3]: {hoVaTen, diemTichLuy(int), soSPMua(int)}
     */
